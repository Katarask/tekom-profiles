import { MapPin, Mail, Phone, Shield, Clock } from "lucide-react";
import { getKandidatByToken, getPageContent, isProfileExpired } from "@/lib/notion";
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 flex items-center justify-center">
      <div className="max-w-md mx-auto px-6 text-center">
        ...
        <p className="text-slate-400 mb-8">
          Dieses Kandidatenprofil ist abgelaufen und steht nicht mehr zur Verfügung.
          Bitte kontaktieren Sie uns für aktuelle Informationen.
        </p>
        <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-800">
          <p className="text-sm text-slate-500 mb-2">Kontakt</p>
          <p className="text-slate-300">{tekom.email}</p>
          <p className="text-slate-300">{tekom.phone}</p>
        </div>
...
export default async function TokenProfilePage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;

  // Kandidat per Token suchen
  const kandidat = await getKandidatByToken(token);
  ...
  if (isProfileExpired(kandidat.gueltigBis)) {
    return <ExpiredProfile />;
  }

  // Content laden
  const content = await getPageContent(kandidat.id);

  // Profil-ID generieren
  const profileId = `TC-${new Date().getFullYear()}-${kandidat.id.slice(0, 8).toUpperCase()}`;

  // Ablaufdatum formatieren
  const expiryFormatted = kandidat.gueltigBis
    ? new Date(kandidat.gueltigBis).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric' })
    : null;
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-12 py-8 sm:py-12 lg:py-20">
        ...
        <section className="py-8 sm:py-12 lg:py-16 border-b border-slate-800 fade-in fade-in-2">
          <div className="flex items-center gap-3 mb-4 sm:mb-6">
            <p className="text-xs tracking-[0.3em] text-teal-500 uppercase">{profileId}</p>
            {expiryFormatted && (
              <span className="text-xs text-slate-600 flex items-center gap-1">
                <Clock className="w-3 h-3" />
                Gültig bis {expiryFormatted}
              </span>
            )}
          </div>
